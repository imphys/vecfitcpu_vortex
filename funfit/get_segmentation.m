function [allrois,roixy,framelist,params] = get_segmentation(params,datain,thr_detection)
% This function converts frames to rois using fast peak detection
...
try
    ...
    allframes=zeros(InfoImage(1).Width,InfoImage(1).Height,Nframes);
    for j=1:Nframes
        allframes (:,:,j) = double(imread(datain, j));
    end
end
...
if strcmp(params.excitation,'zstack')
    allrois = zeros(roisize,roisize,size(allframes_tmp,3),numel(ftmp)-1);
else
    allrois = zeros(roisize,roisize,K,numel(ftmp)-1);
end
...
for ff = 1:size(allframes,3)/K
    ...
    while ftmp(count) == ff
        ...
        if strcmp(params.excitation,'zstack')
            allrois(:,:,:,count) = allframes_tmp(yrange,xrange,:);
        else
            allrois(:,:,:,count) = allframes(yrange,xrange,idx);
        end
...
fprintf(['Found ' num2str(size(allrois,4)) ' spots in ' num2str(toc,3) 's\n'])

params.FOV = FOV;
params.Ncfg = size(allrois,4);
params.K = size(allrois,3);
